...
// Nota: Los botones de cambio de capa no están en el nuevo HTML,
// pero la función se queda por si los quieres añadir.
...
function inicializarControles() {
    // ¡Poblar los checkboxes de criterios dinámicamente desde PHP!
...
function popularCheckboxes(criteriaArray) {
    ...
    criteriaArray.forEach(criterio => {
        ...
        // Reemplazar _ con espacio para legibilidad
 ...
 * (Modificada para usar los datos JSON de PHP)
 */
function actualizarMapa() {
    ...
    // --- 2. Claves de acceso (siempre son las mismas ahora) ---
    ...
    ALL_MAP_DATA.forEach(item => {
        ...
        let metCriteriaNames = []; // Array para guardar nombres
        
        criteriosChecked.forEach(criterio => {
            // La lógica del 'Sí' / 'si' es la misma,
            // ya que nuestro modelo PHP (CrossReference) genera 'Sí'
            if (item[criterio]) {
                let valor = item[criterio].toString().trim().toLowerCase();
                if (valor === 'si' || valor === 'sí') {
                    criteriosCumplidos++;
                    metCriteriaNames.push(criterio.replace(/_/g, ' ')); // Guardamos el nombre legible
                }
            }
        });

        // Guardar el item, su conteo Y la lista de criterios
        datosFiltrados.push({ 
            item: item, 
            count: criteriosCumplidos,
            metCriteria: metCriteriaNames // Guardamos la lista
        });
    ...
    // --- 4. Determinar el "piso" de criterios a mostrar (lógica idéntica a tu JS) ---
    let conteoMinimo = 1; 
    ...
    datosParaExportar = []; // Limpiar array global

    datosFiltrados.forEach(data => {
        const { item, count, metCriteria } = data; // Obtenemos toda la info

        // Aplicar el filtro de conteo
        if (count >= conteoMinimo) {
            
            // --- A. DIBUJAR PIN EN MAPA ---
            // Los datos ya vienen con lat/lng del MapModel.php
            ...
            popupContent += metCriteria.join('<br>'); // Usar la lista 'metCriteria'

            marker.bindPopup(popupContent);
            marker.addTo(markersLayer);

            // --- B. AÑADIR FILA A LA TABLA HTML ---
            const criteriaListString = metCriteria.join('; '); // Usar ; para la tabla
            tableHtml += `<tr>
            ...
            datosParaExportar.push({
                ...
                conteo: count,
                criterios: criteriaListString // Usar la misma lista con ;
            });
        }
    });
 ...
 * (Esta función es idéntica a la tuya)
 */
...
function getPinStyle(count) {
    ...
    switch (count) {
        ...
        case 1: color = '#4CAF50'; break; // Verde
